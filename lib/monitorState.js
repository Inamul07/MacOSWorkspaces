...
export class MonitorStateManager {
    ...
    constructor(interop) {
        ...
        this._syncMonitors();
        debug(`state initialised — ${this.describe()}`);
    }
    ...
    getVirtualIndex(monitorIndex) {
        ...
        if (index === undefined) {
            warn(
                `no state for monitor ${monitorIndex}, assuming 0`);
            return 0;
        }
        return index;
    }
    ...
    setVirtualIndex(monitorIndex, workspaceIndex) {
        if (!this._state.has(monitorIndex)) {
            warn(
                `ignoring state for unknown monitor ${monitorIndex}`);
            return 0;
        }
    ...
    _onMonitorsChanged() {
        this._syncMonitors();
        debug(`monitors changed — ${this.describe()}`);
    }
    ...
    _onWorkspaceCountChanged() {
        ...
        debug(`workspace count changed — ${this.describe()}`);
    }
}
