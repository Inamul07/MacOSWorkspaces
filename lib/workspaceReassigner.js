...
import {shellFromVirtual} from './windowTracker.js';
...
export class WorkspaceReassigner {
     ...
     */
    constructor({interop, monitorState, windowTracker}) {
    ...
    stageMonitor(monitorIndex) {
        ...
        this._windowTracker.suppress(() => {
            for (const [window, record] of this._windowTracker.forMonitor(monitorIndex)) {
                const offset = record.virtualWorkspace - virtualIndex;
    ...
    disable(reason) {
        ...
        this.restoreAll();
        this._enabled = false;
        console.warn(`[macos-workspaces] per-monitor persistence off — ${reason}`);
    }
    ...
    syncMonitor(monitorIndex, activeIndex) {
        ...
        console.log(`[macos-workspaces] monitor ${monitorIndex} showing virtual ` +
    ...
    restoreAll() {
        ...
        if (moved)
            console.log(`[macos-workspaces] restored ${moved} window(s) to their own workspace`);
    }
