...
export function createInterop() {
    return {
        ...
        /**
         * Runs a callback once the Shell has finished the work it is doing now.
         *
         * `monitors-changed` is emitted before mutter has moved windows off a
         * display that has gone away, so anything that asks which monitor a
         * window is on has to wait for that to finish. Low priority puts this
         * behind the relayout rather than in front of it.
         *
         * @param {Function} callback - work to run later
         * @returns {number} a source id, for `cancelDeferred`
         */
        defer: callback => GLib.idle_add(GLib.PRIORITY_LOW, () => {
            callback();
            return GLib.SOURCE_REMOVE;
        }),

        /**
         * Cancels work queued with `defer` that has not run yet.
         *
        ...
        /**
         * UUIDs of every extension GNOME is currently set to load.
         *
         * @returns {string[]} enabled extension UUIDs
         */
        getEnabledExtensions: () => {
            const shell = new Gio.Settings({schema_id: 'org.gnome.shell'});
            return shell.get_strv('enabled-extensions');
        },

        /**
         * Watches the two mutter preferences that decide whether persistence
         * may run at all.
         *
         * `Meta.prefs_get_*` reads the current value but offers GJS no change
         * notification, so the settings themselves are watched instead.
         *
         * @param {Function} callback - invoked with the changed key's name
         * @returns {Function} call to stop watching
         */
        watchPreferences: callback => {
            const mutter = new Gio.Settings({schema_id: 'org.gnome.mutter'});
            const ids = [
                mutter.connect('changed::dynamic-workspaces',
                    () => callback('dynamic-workspaces')),
                mutter.connect('changed::workspaces-only-on-primary',
                    () => callback('workspaces-only-on-primary')),
            ];

            return () => {
                for (const id of ids)
                    mutter.disconnect(id);
            };
        },
