...
export class WindowTracker {
    ...
    constructor({interop, monitorState}) {
        ...
        debug(`tracking ${this._records.size} windows`);
    }
    ...
    /**
     * Re-attributes every tracked window to a monitor, from scratch.
     *
     * Monitor indices are positional, so unplugging a display renumbers the ones
     * that remain and every record suddenly names the wrong monitor. Rebuilding
     * is the only honest answer: the geometry that produced the old attribution
     * no longer exists.
     *
     * Each window's virtual workspace is preserved where its monitor survived,
     * and reset to whatever its new monitor is showing where it did not — a
     * window that has moved to a different display belongs to that display's
     * current workspace, which is exactly what the user sees.
     *
     * @returns {number} how many windows are tracked afterwards
     */
    retrackAll() {
        const previous = new Map();
        for (const [window, record] of this._records)
            previous.set(window, record);
        ...
        for (const window of this._interop.getWindows()) {
            ...
            const before = previous.get(window);
            const record = this._records.get(window);
            if (before && before.monitor === record.monitor)
                record.virtualWorkspace = before.virtualWorkspace;
        }

        // A window that was tracked before and is not now maps to no monitor at
        // all, which means the caller ran this before mutter had finished moving
        // windows off the display that went away. Say so: silently forgetting a
        // window is how one ends up parked on a workspace nobody can reach.
        const lost = [...previous.keys()].filter(w => !this._records.has(w)).length;
        debug(`re-attributed ${this._records.size} window(s) ` +
            `after a monitor change${lost ? `, lost ${lost}` : ''}`);
        return this._records.size;
    }
