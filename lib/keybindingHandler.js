...
export class KeybindingHandler {
    ...
    constructor({interop, monitorState, driver}) {
        ...
        debug(`keybindings taken over (${this._taken.length})`);
    }
    ...
    _onSwitch(binding) {
        ...
        if (delta === 0) {
            if (BINDINGS.has(name)) {
                debug(`key: ${name} does not apply to the ` +
        ...
        // Resolve before opening a switch: a keystroke at either end of the
        // strip must not stage windows or build animation actors for a move
        // that is not going to happen.
        const from = this._driver.virtualIndexOf(monitor);
        const to = this._driver.resolveVirtual(monitor, from, delta);

        if (to === from) {
            // Stock GNOME also refuses to go further, but say so rather than
            // doing nothing silently.
            debug(`key: monitor ${monitor} already at ` +
                `workspace ${from}, no move (turn on wrap-around to continue past the end)`);
            return;
        }
        ...
        // A switch is measured from the anchor it opened on, not from wherever
        // the last keypress left the index. Pressing again mid-settle therefore
        // adds to how far this switch has already travelled — otherwise the
        // second press repeats the first instead of continuing it.
        this._driver.settle(session, session.step + delta);
    }
}
