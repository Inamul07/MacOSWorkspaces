...
export class GestureHandler {
    ...
    destroy() {
        ...
        if (wac) {
            ...
            error(
                'cannot restore gesture handlers — controller gone');
        }

        this._tracker = null;
        this._session = null;
        debug('gesture handlers restored');
    }
    ...
    _takeOver() {
        if (!this._tracker) {
            error('no swipe tracker — gestures not intercepted');
            return;
        }
        ...
        for (const name of SIGNALS) {
            ...
            if (removed !== 1) {
                warn(`expected 1 '${name}' handler on the ` +
        ...
        debug('swipe tracker taken over');
    }
    ...
    _disconnectSignal(name) {
        ...
        if (!signalId) {
            warn(`swipe tracker has no '${name}' signal`);
            return 0;
        }
